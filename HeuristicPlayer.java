...
//Basic Constructor
	...
	double[] evaluate(int currentPos, int dice, int  opponentPos) {//συναρτηση αξιολογησης της εκάστοτε κίνησης 
		...
		double distO = 0;//αποστασή παίκτη απο τον αντίπαλο
	    ...
	    NearSupplies= 1.0/distS;//πόντοι για εφόδιο- όσο πιο κοντά τόσο πίο πολλοί πόντοι-1.0/dist είναι 1,0.5,0.333 γιατι είναι τύπου double
	...
	//όμοια λογική -το i εδώ μειώνεται κατά 1 ώστε να ψάχνει προς τα αριστερα 
		...
		if(sameDices>=3 && d0 <=0) {
			d0 = d0-0.04;//αν επαναλαμβάνει μία κατέυθυνση και δεν βλέπει εφόδιο να μην την ξαναπροτιμήσει	
			}
	...
	int getNextMove(int currentPos,int opponentPos) {
	     ...
	     if(eval[1] == 1.0/3.0) path.add(3);//8 αποσταση απο εφοδιο αν εβλεπε
	     else if(eval[1] == 0.5) path.add(2);
	     else if(eval[1] == 1) path.add(1);
	     if(eval[2] == 1.0/3.0) path.add(3);//9o αποσταση απο μινωταυρο αν εβλεπε-αν δεν εβλεπε εφοδιο και εβλεπε μινωταυρο θα ταν 8ο
	...
	public void statistics() {
		...
		int start = -2;
		int movement=0;
		int round = -1;//ώστε με το την πρώτη κίνηση να αυξήθει 1 -Θησέας
	    if(this.playerId ==2) round =0;//ώστε με το την πρώτη κίνηση να αυξήθει 2-Mινώταυρος 
	
		for(int  i =0;  i <path.size()-8; i++) {//για όσες κινήσεις έχουν γίνει ξάχνει το σήμειο εκκίνησης -1 ώστε να αυξήσει το movement 
			//-εκτελείται μέχρι -8 ώστε να μην μετρήσει κα ιτο -1 το τέλους της τελευταίας κίνησης
				if(path.get(i) == -1) {
					...
					movement++;//κίνηση παίκτη
					round++;//o γύρος στον οποίο έπαιξε 
					
				}
				if(start!= -2) {//για κάθε κίνηση που εχει βρεθει
				statiInf.add(this.name+"'s movement is "+movement+" - Round: "+round);//o αριθμός  της κίνησης του παίκτη και ο γύρος που έπαιξε
				if(this.playerId == 1) statiInf.add("Τheseus collected "+this.score+" supplies" );// μηνυμα  για το πόσα εφόδια έχει μαζέψει 
				if(path.get(start+6) == 1) {//αν έβλεπε εφόδιο 
				statiInf.add("Distance from supply was "+path.get(start+8));//μηνυμα για την αποσταση απο εφόδιο εαν εβλέπε καποιο
				if(path.get(start+7) == 1) statiInf.add("Distance from opponent was "+path.get(start+9));//μηνυμα για την αποσταση απο τον εχθρο αν εβλεπε και εχθρο
				}else if(path.get(start+7) == 1) statiInf.add("Distance from opponent was "+path.get(start+8));//αν εβλεπε εχθρο αι οχι εφοδιο μηνυμα για την αποσταση απο αυτον
